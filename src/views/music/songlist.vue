...
// 编辑表单
const editPlaylistForm = ref({ name: '', description: '', coverUrl: '' })
...
function getPlaylistCover(playlist: PlaylistRow): string {
  if (playlist.coverImgUrl && playlist.coverImgUrl !== 'default-cover')
...
const editPlaylist = (playlist: PlaylistRow) => {
  ...
  editPlaylistForm.value = {
    ...
    coverUrl: playlist.coverImgUrl && playlist.coverImgUrl !== 'default-cover' ? playlist.coverImgUrl : ''
  }
  showEditPlaylistDialog.value = true
}

const handleEditCoverUpload = (e: Event) => {
  const file = (e.target as HTMLInputElement).files?.[0]
  if (!file) return
  if (file.size > 5 * 1024 * 1024) {
    MessagePlugin.warning(t('music.list.imageTooLarge'))
    return
  }
  const reader = new FileReader()
  reader.onload = () => {
    editPlaylistForm.value.coverUrl = reader.result as string
  }
...
const savePlaylistEdit = async () => {
  ...
  try {
    ...
    if (ok) {
      // 更新封面
      if (editPlaylistForm.value.coverUrl) {
        await localUserStore.updatePlaylistCover(currentEditingPlaylist.value.id, editPlaylistForm.value.coverUrl)
      }
  ...
  <div ref="scrollRef" class="page">
    ...
    >
      <div class="edit-playlist-content">
        <div class="form-item cover-upload-item">
          <label class="form-label">{{ t('music.list.editCover') }}</label>
          <div class="cover-upload-area" @click="editCoverFileInputRef?.click()">
            <img v-if="editPlaylistForm.coverUrl" :src="editPlaylistForm.coverUrl" class="cover-preview" />
...
.cover-upload-area:hover { border-color: var(--td-brand-color); }
.cover-preview {
  width: 100%;
  height: 100%;
  object-fit: cover;
...
.cover-placeholder .iconfont { font-size: 24px; }
