...
const handleAddToPlaylist = (song: MusicItem) => {
  ...
  isMovingSong.value = false
  showAddToPlaylist.value = true
}

const handleMoveToPlaylist = (song: MusicItem) => {
  songsToAdd.value = [song as any]
  isMovingSong.value = true
  showAddToPlaylist.value = true
}
...
const handleSongAddedToPlaylist = async (targetPlaylistId: string) => {
  if (!isMovingSong.value) return
  isMovingSong.value = false
  const song = songsToAdd.value[0]
  if (!song) return
  try {
    await localUserStore.removeSongFromPlaylist(playlistInfo.value.id, String(song.songmid))
    await fetchSongs()
  } catch {
    console.warn('[list] 移动歌曲失败')
  }
}
    ...
    <div class="scrollable-content">
      ...
      <div v-else class="song-list-wrapper">
        <SongVirtualList
          ...
          @move-to-playlist="handleMoveToPlaylist"
