...
const onToggleLike = async (song: MusicItem) => {
  try {
    ...
    if (isLiked(song)) {
      ...
      // 如果当前正在查看收藏歌单，通知父组件移除该歌曲
      if (props.playlistId === favId) {
        emit('removeFromPlaylist', song)
      }
    } else {
...
const handleMenuAction = (action: string) => {
  ...
  else if (action === 'moveToPlaylist') emit('moveToPlaylist', song)
  ...
  <div class="song-virtual-list">
    ...
    <Teleport v-if="contextMenuVisible" to="body">
      ...
      >
        ...
        <div v-if="isLocalPlaylist" class="ctx-item" @click="handleMenuAction('moveToPlaylist')">
          <i class="iconfont icon-zengjia"></i> {{ t('music.songList.moveToPlaylist') }}
        </div>
